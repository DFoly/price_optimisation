...
The databricks CLI bundles top level is defined by file `price_optimisation/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`price_optimisation/resources/model-workflow-resource.yml`)
 - batch inference workflow (`price_optimisation/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`price_optimisation/resources/monitoring-resource.yml`)
 - feature engineering workflow (`price_optimisation/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`price_optimisation/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `price_optimisation/databricks.yml` and `price_optimisation/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-price_optimisation-model     | /dev-price_optimisation-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-price_optimisation-model | /staging-price_optimisation-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-price_optimisation-model    | /prod-price_optimisation-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-price_optimisation-model    | /test-price_optimisation-experiment    |
...
2. Complete the `TODO` in `price_optimisation/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `price_optimisation/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `price_optimisation/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-price_optimisation-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-price_optimisation-batch-inference-job`
that runs the notebook under `price_optimisation/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-price_optimisation-batch-inference-job`.
...
`test-price_optimisation-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-price_optimisation-batch-inference-job`.
